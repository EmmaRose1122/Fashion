...
const categoryInfo = CATEGORIES.find((c) => c.slug === "beauty")!;
...
export default async function BeautyCategoryPage({ searchParams }: CategoryPageProps) {
  const resolvedParams = await searchParams;
  const currentPage = Number(resolvedParams.page) || 1;

  const totalArticles = await prisma.article.count({
    where: { category: "beauty", published: true },
  });
